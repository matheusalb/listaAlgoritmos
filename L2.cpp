...
struct andar{
    struct allBooks livros[496];
    int nLivrosTotais;
};

class armario{
    public:
        struct andar andares[11]; //pra cada andar
        struct allBooks infoGerais[2][496][11]; //se idbd[0]<5 elemento ta no infogerais[0] , caso contrario infogerais[1]
        armario(){                 //( tipo1 isbn[0]<5) (tipo2 soma ponderada) (tipo 3 dv)
            int j,k,i;
            for(i = 0; i<11; i++){
                this -> andares[i].nLivrosTotais = 0;
                for(j=0;j < 496 ;j++){
                    
                    this -> andares[i].livros[j].livros = new struct livro [10];
                    this -> andares[i].livros[j].nLivrosDiferentes = 0;
                    this -> andares[i].livros[j].qAlocado = 10;
        ...
        int addLivros(string& id,int tipo, int quantidade, int limitePorAndar, int dv, int* nPrateleiras,int tipo2){
            ...
            for(j=0;j < 11 && quantidade > 0; j++){
                andarAtual = (dv + j)% 11;
                index = this -> busca_binaria(andares[andarAtual].livros[tipo2].livros,id,&ultimoIndex,andares[andarAtual].livros[tipo2].nLivrosDiferentes); //indice do elemento do vetor que contem o livro no andar
                ...
                if(index > -1){   //já existe livro nesse andar
                    ...
                    if(maxAddPorAndar > 0){ //posso adicionar nesse andar
                        ...
                        if(andares[andarAtual].livros[tipo2].livros[index].quantidade == 0)
                            *nPrateleiras = *nPrateleiras + 1;

                        if(quantidade <= maxAddPorAndar){ //posso add tudo nesse andar
                            this->andares[andarAtual].livros[tipo2].livros[index].quantidade += quantidade;
                            this->andares[andarAtual].nLivrosTotais += quantidade;
                            quantidade = 0;
                        }
                        else{
                            this->andares[andarAtual].livros[tipo2].livros[index].quantidade += maxAddPorAndar;
                    ...
                    if(maxAddPorAndar > 0){ //posso add
                        ...
                        if(this->andares[andarAtual].livros[tipo2].qAlocado == this->andares[andarAtual].livros[tipo2].nLivrosDiferentes){
                            andares[andarAtual].livros[tipo2].livros = this -> dobra_vetor(this->andares[andarAtual].livros[tipo2].livros, &(this->andares[andarAtual].livros[tipo2].qAlocado),this->andares[andarAtual].livros[tipo2].nLivrosDiferentes); 
                        }               
                        ...
                        *nPrateleiras = *nPrateleiras + 1;
                        ...
                        if(quantidade <= maxAddPorAndar){
                            this->andares[andarAtual].livros[tipo2].livros[novoIndex].quantidade += quantidade;
                            this->andares[andarAtual].nLivrosTotais += quantidade;
                            quantidade = 0;
                        }
                        else{
                            this->andares[andarAtual].livros[tipo2].livros[novoIndex].quantidade += maxAddPorAndar;
                            this->andares[andarAtual].nLivrosTotais += maxAddPorAndar;
                            quantidade = quantidade - maxAddPorAndar;
                        }
                    }
        ...
        int criaLivro(string& id,int andar,int ultimoIndex,int* indexVetor, bool cria,int dv,int tipo,int tipo2){
            ...
            for(int i = ultimoIndex; i < this->andares[andar].livros[tipo2].nLivrosDiferentes; i++){

                if(this->andares[andar].livros[tipo2].livros[i].ID > livroADD.ID){
                    if(livroADD.ID == id && !entrou){
                        entrou = true;
                        novoIndex = i;
                    }
                    aux = livroADD;
                    livroADD = this->andares[andar].livros[tipo2].livros[i];
                    this->andares[andar].livros[tipo2].livros[i] = aux;
                }
            }
            if(novoIndex == -1)
                novoIndex = this->andares[andar].livros[tipo2].nLivrosDiferentes;
            this->andares[andar].livros[tipo2].livros[ this->andares[andar].livros[tipo2].nLivrosDiferentes ] = livroADD;
            this->andares[andar].livros[tipo2].nLivrosDiferentes++;
        ...
        int removeLivros(string& id, int quantidade, int dv, int* nPrateleiras,int tipo,int tipo2){
            ...
            for(int j = 0; j < 11 && quantidade > 0; j++){
                ...
                index = this -> busca_binaria(this->andares[andar].livros[tipo2].livros,id,&ultimoIndice,this->andares[andar].livros[tipo2].nLivrosDiferentes);

                if(this->andares[andar].livros[tipo2].livros[index].quantidade >= quantidade){

                    this->andares[andar].livros[tipo2].livros[index].quantidade -= quantidade;
                    this->andares[andar].nLivrosTotais -= quantidade;
                    quantidade = 0;

                    if(this->andares[andar].livros[tipo2].livros[index].quantidade == 0){
                        *nPrateleiras = *nPrateleiras - 1;
                        this->andares[andar].livros[tipo2].livros[index].nPrateleiras = *nPrateleiras;
                    }
                }
                else if(this->andares[andar].livros[tipo2].livros[index].quantidade > 0){
                    this -> andares[andar].nLivrosTotais -= this->andares[andar].livros[tipo2].livros[index].quantidade;
                    quantidade = quantidade - this->andares[andar].livros[tipo2].livros[index].quantidade;
                    this -> andares[andar].livros[tipo2].livros[index].quantidade = 0;
                    *nPrateleiras = *nPrateleiras - 1;
                    this -> andares[andar].livros[tipo2].livros[index].nPrateleiras = *nPrateleiras;
                }
